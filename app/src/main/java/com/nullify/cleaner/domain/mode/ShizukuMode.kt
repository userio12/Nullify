...
class ShizukuMode : ExecutionMode {

    override val name: String = "Shizuku"
    override val level: ModeLevel = ModeLevel.SHIZUKU
    ...
    override suspend fun listDirectory(path: String): Result<List<String>> = withContext(Dispatchers.IO) {
        runCatching { shell("ls -1 \"$path\"").getOrThrow().lines().filter { it.isNotBlank() } }
    }

    override suspend fun fileExists(path: String): Boolean = withContext(Dispatchers.IO) {
        runCatching { shell("test -e \"$path\" && echo true || echo false").getOrThrow() == "true" }.getOrDefault(false)
    }
}
